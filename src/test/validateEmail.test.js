...
test('Pruebas para validateEmail', async (t) => {
    await t.test('debería retornar "admin" para correos que terminen en @admin.com', () => {
        const result = validateEmail('test@admin.com');
        assert.strictEqual(result, 'admin');
    });

    await t.test('debería retornar "superadmin" para correos que terminen en @superadmin.com', () => {
        const result = validateEmail('test@superadmin.com');
        assert.strictEqual(result, 'superadmin');
    });

    await t.test('debería retornar "user" para cualquier otro dominio de correo', () => {
