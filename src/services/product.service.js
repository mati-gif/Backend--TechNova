...
export const findById = async (req, res) => {
  try {
    ...
    if (!product) {
      ...
      return res
        .status(404)
        .send({ message: "No se encontró el producto solicitado" });
    }
...
export const createNewProduct = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: `¡El producto ${existProduct.name} ya existía, se incrementó una unidad el stock!`,
      existProduct,
    });
  } catch (error) {
    console.error(error);
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      ...
      return res
        .status(404)
        .json({ message: "No se encontró el producto solicitado" });
    }
    ...
    return res.status(200).json({
      message: `¡El producto ${product.name} fue actualizado con exito!`,
      product: product.id,
    });
  } catch (error) {
...
export const deleteAProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      console.log(
        `No se encontró el producto con el id ${id} para dar de baja`,
      );
      return res
        .status(404)
        .json({ message: "No se encontró el producto solicitado" });
    ...
    if (!product.isActive) {
      return res
        .status(400)
        .json({ message: "El producto ya se encuentra dado de baja" });
    }
