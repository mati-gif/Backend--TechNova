...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!address)
      return res.status(404).json({ message: "Dirección inválida" });
    ...
    const orderCode = `TN-${new Date().getFullYear()}-${String(nextNumber).padStart(3, "0")}`;
    ...
    res
      .status(201)
      .json({ message: "Orden creada con éxito", order: newOrder });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error al crear la orden: " + error.message });
  }
};
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Listado de pedidos para usuarios activos", orders });
  } catch (error) {
...
export const getOrderById = async (req, res) => {
  try {
    ...
    if (!order || {})
      return res
        .status(404)
        .json({ message: "Orden no encontrada o usuario inactivo" });
