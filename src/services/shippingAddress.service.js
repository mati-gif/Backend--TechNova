  ...
  try {
      ...
      include: [
        {
          ...
          // Filtro directo: solo usuarios activos
          where: { active: true }, 
        },
      ],
    });
    ...
    if (!addresses) {
      return res.status(404).json({
        message: "No se encontró una dirección que tenga su usuario activo",
      });
    }
    ...
    res
      .status(500)
      .send({ message: "Ocurrió un error al traer las direcciones" });
  }
};
  ...
  try {
    ...
    const address = await ShippingAddress.findByPk(id, {
      include: [
        {
          model: User,
          // Si el usuario no está activo, la consulta fallará o no traerá nada
          where: { active: true }, 
        },
      ],
    });
    ...
    if (!address)
      return res
        .status(404)
        .json({ message: "Dirección no encontrada o usuario deshabilitado" });
...
export const createShippingAddress = async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: `Nueva dirección creada para el usuario ${user.email}`,
      newAddress: newAddress,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error al crear la dirección: " + error.message });
  }
};
...
export const getAllShippingAddressesByUserId = async (req, res) => {
  try {
    ...
    // Buscamos las direcciones filtrando directamente por el userId (incluimos el usuario para verificar que esté activo)
    const addresses = await ShippingAddress.findAll({
      where: {
        userId: userId,
      },
    ...
    if (addresses.length === 0) {
      return res
        .status(404)
        .json({
          message: "No se encontraron direcciones para este usuario activo.",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Error al obtener las direcciones: " + error.message });
  }
};
